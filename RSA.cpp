...
#define BTN_BG          RGB(51, 51, 55)
#define BTN_HOVER       RGB(80, 80, 85)
#define BTN_DOWN        RGB(0, 122, 204)
#define BORDER_COLOR    RGB(85, 85, 85)
...
HWND hLblD, hEdtD;
HWND hLblEnc, hBtnEnc, hLblEncRes;
HWND hLblDec, hBtnDec, hLblDecRes;
HWND hEdtInEnc, hEdtOutEnc, hEdtInDec, hEdtOutDec;
...
void CreateUI(HWND hWnd) {
    ...
    hEdtInEnc = CreateStdEdit("Halo Kriptografi RSA!", hWnd, hFont, ID_EDT_IN_ENC, ES_MULTILINE | ES_WANTRETURN | ES_AUTOVSCROLL | WS_VSCROLL, false);
    ...
    hLblDec = CreateStdLabel("Algoritma Dekripsi:", hWnd, hTitleFont, false);
...
void ResizeLayout(int width, int height) {
    ...
    int margin_r = 50; // Jarak pinggir kanan ekstra (ruang bebas agar scrollbar master tidak numpuk)
    ...
    // Seksi Enkripsi (Kiri) dan Dekripsi (Kanan) -> Statis & Tinggi
    int yEnc = yRow3 + 65;
    int editH = 140; // Kotak yang fix besar agar optimal
    
    MoveWindow(hLblEnc, col1_x, yEnc, wHalf, 25, TRUE);
    MoveWindow(hEdtInEnc, col1_x, yEnc + 25, wHalf, editH, TRUE);

    int yBtnBase = yEnc + 25 + editH + 15;
    MoveWindow(hBtnEnc, col1_x, yBtnBase, 150, 35, TRUE);
    MoveWindow(hLblEncRes, col1_x, yBtnBase + 45, wHalf, 20, TRUE);
    MoveWindow(hEdtOutEnc, col1_x, yBtnBase + 70, wHalf, editH, TRUE);

    // Seksi Dekripsi (Kanan)
    MoveWindow(hLblDec, col2_x, yEnc, wHalf, 25, TRUE);
    MoveWindow(hEdtInDec, col2_x, yEnc + 25, wHalf, editH, TRUE);

    MoveWindow(hBtnDec, col2_x, yBtnBase, 150, 35, TRUE);
    MoveWindow(hLblDecRes, col2_x, yBtnBase + 45, wHalf, 20, TRUE);
    MoveWindow(hEdtOutDec, col2_x, yBtnBase + 70, wHalf, editH, TRUE);
}
...
void UpdateScrollState(HWND hWnd) {
    ...
    if (g_isAppView) {
        si.nMax = 750; // Jarak total logical element saat aktif App
        si.nPos = 0;
        si.fMask |= SIF_POS;
    } else {
    ...
    case WM_COMMAND: {
        switch (LOWORD(wParam)) {
            ...
            case ID_BTN_ENCRYPT: {
                ...
                // Concat nilai Desimal murni
                std::string concatAscii = "";
                for (char c : msg) {
                    concatAscii += std::to_string((unsigned char)c);
                }
                
                // Pecah jadi 3 digit
                std::vector<std::string> chunks;
                for (size_t i = 0; i < concatAscii.length(); i += 3) {
                    chunks.push_back(concatAscii.substr(i, 3));
                }
                
                std::string asciiStr = "Desimal ASCII (Blok 3-Digit): ";
                std::vector<uint64> encrypted;
                
                for (size_t i = 0; i < chunks.size(); i++) {
                    std::string ch = chunks[i];
                    if (ch.length() < 3) {
                        // Jika blok terakhir kurang dari 3 digit, tambahkan 0 di kiri
                        ch = std::string(3 - ch.length(), '0') + ch;
                    }
                    asciiStr += ch + " ";
                    uint64 m_val = std::stoull(ch);
                    encrypted.push_back(modPow(m_val, input_e, input_n));
                }
                
                std::string cipherStr = cipherToString(encrypted);
                std::string finalOutput = asciiStr + "\r\n---------------------------------------\r\nCiphertext: " + cipherStr;
                
                SetWindowTextA(hEdtOutEnc, finalOutput.c_str());
            ...
            case ID_BTN_DECRYPT: {
                ...
                std::string asciiStr = "Desimal ASCII kembali: ";
                std::string concatAscii = "";
                ...
                for (size_t i = 0; i < encrypted.size(); ++i) {
                    ...
                    char buf[16];
                    sprintf(buf, "%03llu", decVal);
                    asciiStr += std::string(buf) + " ";
                    
                    if (i == encrypted.size() - 1) {
                         // Blok terakhir diekstrak as-is tanpa padding (3 bukan 003) untuk merekonstruksi string persis seperti awal
                         concatAscii += std::to_string(decVal);
                    } else {
                         concatAscii += std::string(buf);
                    }
                }
                
                std::string decryptedMsg = "Pesan Asli: ";
                // Decode pasangan per 2-digit untuk karakter normal
                for (size_t i = 0; i < concatAscii.length(); i += 2) {
                    if (i + 1 < concatAscii.length()) {
                        decryptedMsg += (char)std::stoi(concatAscii.substr(i, 2));
                    } else {
                        decryptedMsg += (char)std::stoi(concatAscii.substr(i, 1));
                    }
                }
                
                std::string finalOutput = asciiStr + "\r\n---------------------------------------\r\n" + decryptedMsg;
                
                SetWindowTextA(hEdtOutDec, finalOutput.c_str());
                break;
            }
        }
        break;
